...
$(function() {
  var age, allCategories, asin, author, authorExpander, authorRank, catTableButton, categories, close, d, fetchAllCategories, fileSize, info, isEbook, length, num, pubDate, pubDateRaw, publisher, rank, ratingAvg, ratingCount, removeBtn, tier, words;
  ...
  ratingAvg = num($('#summaryStars a.product-reviews-link').attr('title') || $('#revFMSR a').attr('title') || 0);
  ...
  info.append([
    ...
    })(), ' - ', 'Author: ', $('<span class=authors/>').append(author), authorExpander, length ? (" - Length: " + length + " (~" + (words.toLocaleString()) + " words)") + '<sup><abbr title="Number of pages times 250 words per page">?</abbr></sup>' : void 0, fileSize ? " - Size: " + fileSize : void 0, ' - ', 'ASIN: ', asin, '<br/>', authorRank, 'Book Rank: #', rank.toLocaleString(), ' - ', 'Tier ', tier, '<sup><abbr title="From Chris Fox\'s &quot;Writing To Market&quot;">?</abbr></sup>', ' - ', "<a href='https://www.novelrank.com/asin/" + asin + "'>NovelRank</a>", ' - ', "<a href='https://kindlepreneur.com/amazon-kdp-sales-rank-calculator/#" + rank + "," + (isEbook ? 1 : 0) + "'>KP</a>", ' - ', "<a href='http://www.tckpublishing.com/amazon-book-sales-calculator/#" + rank + "," + (isEbook ? 1 : 0) + "'>TCK</a>", ' - ', 'Rating: ', ratingAvg, ' - ', 'Reviews: ', "<a href=#customerReviews>" + (ratingCount.toLocaleString()) + "</a>", ' - ', 'Age: ', (Math.round(age.asWeeks())) + " weeks", ' - ', 'Ratio: ', Number(ratingCount / age.asWeeks()).toFixed(2), '<sup><abbr title="Number of ratings divided by the age in weeks">?</abbr></sup>', '<br/>', $('<div class="cat-table"/>').append($('<div class="cat-table-cell"/>').append(categories), $('<div class="cat-table-cell"/>').append(catTableButton))
  ]);
  close = $('<div/>');
  close.css({
    position: 'absolute',
  ...
  fetchAllCategories = function() {
    ...
    return forEachSeries(['BS', 'HNR'], function(mode, outerCb) {
      return forEachSeries(categories.find('li'), function(li, cb) {
        var fetch, host, label, page, protocol, ref2, ref3;
        li = $(li);
        id = Number((ref2 = li.find('a[href^="/"]:last').attr('href').match(/node=(\d+)/)) != null ? ref2[1] : void 0);
